...
export class BotActivityHandler extends TeamsActivityHandler {
  ...
  async processMessage(context: TurnContext, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      if (context.activity.value !== undefined && context.activity.text === undefined) {
        // From mouse clicking
        // Get  event
        const event: IEvent = {
        ...
        // Get router
        const router = this.bot.geRouter() as MsteamsRouter;
        ...
        // From user input
        // Get listeners
        const listeners = this.bot.getListeners() as MsteamsListener[];
  ...
  async handleTeamsTaskModuleFetch(context: TurnContext, taskModuleRequest: TaskModuleRequest): Promise<TaskModuleResponse> {
    ...
    try {
      ...
      const router = this.bot.geRouter() as MsteamsRouter;
      // Call route handler for mouse navigation
      const chatOpsTaskInfo: TaskModuleTaskInfo = (await router.getRoute().handler(chatContextData)) as TaskModuleTaskInfo;
  ...
  async handleTeamsTaskModuleSubmit(context: TurnContext, taskModuleRequest: TaskModuleRequest): Promise<TaskModuleResponse> {
    ...
    try {
      ...
      const taskInfo: TaskModuleTaskInfo = (await router.getRoute().handler(chatContextData)) as TaskModuleTaskInfo;
