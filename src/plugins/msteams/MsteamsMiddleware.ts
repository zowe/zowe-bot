...
export class MsteamsMiddleware extends Middleware {
    ...
    constructor(bot: CommonBot) {
        ...
        // Create adapter
        const msteamsOption: IMsteamsOption = <IMsteamsOption>(
            option.chatTool.option
        );
    ...
    async run(): Promise<void> {
        ...
        try {
            ...
            option.messagingApp.app.post(
                option.messagingApp.option.basePath,
                (req: Request, res: Response) => {
                    this.botFrameworkAdapter.processActivity(
                        req,
                        res,
                        async context => {
    ...
    async send(
        ...
        try {
            ...
            logger.debug(
                `Chat context data sent to MS Teams: ${Util.dumpObject(
                    chatContextData,
                    2
                )}`
            );
            ...
            for (const message of messages) {
                if (message.type === IMessageType.PLAIN_TEXT) {
                    ...
                    message.type === IMessageType.MSTEAMS_ADAPTIVE_CARD
                ) {
                    ...
                    logger.error(
                        `Unsupported type "${
                            message.type
                        }" for the message: ${JSON.stringify(message, null, 2)}`
                    );
                    textMessage = `${textMessage}\n${JSON.stringify(
                        message.message
                    )}`;
                ...
                if (
                    ...
                    for (const mention of message.mentions) {
                        if (
                            mention.mentioned.id.trim() === '' &&
                            mention.mentioned.name.trim() !== ''
                        ) {
                            const channelInfo =
                                this.botActivityHandler.findChannelByName(
                                    mention.mentioned.name
                                );
                            logger.debug(
                                `Channel info for mention ${
                                    mention.mentioned.name
                            ...
                            const channelInfo =
                                this.botActivityHandler.findChannelById(
                    ...
                    logger.debug(
                        `message.mentions: ${JSON.stringify(
                            message.mentions,
                            null,
                            2
                        )}`
            ...
            logger.debug(
                `activity to be sent: ${JSON.stringify(activity, null, 2)}`
            );
                ...
                if (
                    ...
                    const conversationReference =
                        TurnContext.getConversationReference(
                            chatContextData.context.chatTool.context.activity
                        );
                    logger.debug(
                    ...
                    // Send message
                    await this.botFrameworkAdapter.continueConversation(
                        conversationReference,
                        async turnContext => {
                    ...
                    if (
                        chatContextData.context.chatting.channel.id === '' &&
                        chatContextData.context.chatting.channel.name !== ''
                    ) {
                        channelInfo = this.botActivityHandler.findChannelByName(
                            chatContextData.context.chatting.channel.name
                        );
                    } else {
                        channelInfo = this.botActivityHandler.findChannelById(
                            chatContextData.context.chatting.channel.id
                        );
                    }
                    logger.info(
                        `Target channel info: ${JSON.stringify(
                            channelInfo,
                            null,
                            2
                        )}`
                    ...
                    if (channelInfo == null) {
                        logger.error(
                            `The specified MS Teams channel does not exist!\n${JSON.stringify(
                                chatContextData.context.chatting.channel,
                                null,
                                2
                            )}`
                        );
                    ...
                    // Get service URL
                    const serviceUrl = this.botActivityHandler.findServiceUrl(
                        channelInfo.id
                    );
                    logger.info(`Service URL: ${serviceUrl}`);
                    if (serviceUrl === '') {
                        logger.error(
                            `MS Teams service URL does not exist for the channel ${JSON.stringify(
                                channelInfo,
                                null,
                                2
                            )}`
                    ...
                    const connectorClient =
                        this.botFrameworkAdapter.createConnectorClient(
                            serviceUrl
                        );
                    ...
                    if (textMessage !== '') {
                        ...
                        firstActivity = MessageFactory.attachment(
                            attachments[0]
                        );
                        firstActivity.entities = <Entity[]>mentions;
                    }
                    logger.debug(
                        `firstActivity: ${JSON.stringify(
                            firstActivity,
                            null,
                            2
                        )}`
                    ...
                    logger.debug(
                        `restActivity: ${JSON.stringify(restActivity, null, 2)}`
                    );
                    ...
                    this.botFrameworkAdapter.continueConversation(
                        conversationReference,
                        async turnContext => {
