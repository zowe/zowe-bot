...
export class MattermostMiddleware extends Middleware {
    ...
    async run(): Promise<void> {
        ...
        try {
            const mattermostOption = <IMattermostOption>(
                this.bot.getOption().chatTool.option
            );
    ...
    async send(
        ...
        try {
            ...
            logger.debug(
                `Chat tool data sent to Mattermost server: ${Util.dumpObject(
                    chatContextData.context.chatTool,
                    2
                )}`
            );
            ...
            for (const msg of messages) {
                ...
                if (chatContextData.context.chatTool !== null) {
                    ...
                    this.client.sendMessage(
                        ...
                        chatContextData.context.chatTool.rootId
                    );
                } else {
                    // Proactive message
                    ...
                    if (
                        ...
                        const channelInfo: IChannel =
                            await this.client.getChannelByName(
                                chatContextData.context.chatting.channel.name
                            );
    ...
    async processMessage(rawMessage: Record<string, any>): Promise<void> {
        ...
        */
        logger.debug(
            `Message is received from Mattermost server: ${Util.dumpObject(
                rawMessage
            )}`
        );
        ...
        try {
            ...
            if (
                rawMessage.data.channel_type !== undefined &&
                rawMessage.data.channel_type !== null
            ) {
                chattingType = this.client.getChattingType(
                    rawMessage.data.channel_type
                );
            } else {
