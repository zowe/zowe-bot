...
export class SlackMiddleware extends Middleware {
    ...
    constructor(bot: CommonBot) {
        ...
        if (option.chatTool.type !== IChatToolType.SLACK) {
            logger.error(
                `Wrong chat tool type set in bot option: ${option.chatTool.type}`
            );
        ...
        if (
            logOption.level === ILogLevel.DEBUG ||
            logOption.level === ILogLevel.SILLY ||
            logOption.level === ILogLevel.VERBOSE
        ) {
        ...
        if (slackOption.socketMode === false) {
            logger.debug(
                'Socket mode is not enabled, start the http/https receiver'
            );
            ...
            logger.debug(
                `expressReceiverOptions: ${JSON.stringify(
    ...
    async processMessage(
        slackEvent: SlackEventMiddlewareArgs<'message'> & AllMiddlewareArgs
    ): Promise<void> {
        // Print start log
        ...
        try {
            ...
            if (this.botName === undefined || this.botName.trim() === '') {
                const botUserInfo = await slackEvent.client.users.info({
                    user: slackEvent.context.botUserId,
                });
            ...
            let user = this.getUser(
                (<Record<string, any>>slackEvent.message).user
            ); // eslint-disable-line @typescript-eslint/no-explicit-any
            // if user have not been cached, then search from the slack server and cache it
            if (user === undefined) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const userInfo = await slackEvent.client.users.info({
                    user: (<Record<string, any>>slackEvent.message).user,
                });
                logger.debug(
                    `Cache the user info: ${JSON.stringify(userInfo)}`
                );
                user = {
                    id: userInfo.user.id,
            ...
            if (channel == null) {
                channel = await this.getChannelById(
                    channelId,
                    slackEvent.client
                );
            ...
            message = (<Record<string, any>>slackEvent.message).text.replace(
                reg,
                `@${this.botName}`
            ); // eslint-disable-line @typescript-eslint/no-explicit-any
            ...
            let rawMessage = '';
            if (
                (<Record<string, any>>slackEvent.message).blocks !== undefined
            ) {
                // eslint-disable-line @typescript-eslint/no-explicit-any
                const messageBlocks = (<Record<string, any>>slackEvent.message)
                    .blocks; // eslint-disable-line @typescript-eslint/no-explicit-any
                    ...
                    if (
                        ...
                        for (const element of block.elements) {
                            if (
                                ...
                                logger.debug(
                                    'Find block rich_text_section to get the raw message'
                                );
                                for (const richTextElement of element.elements) {
                                    // Only consider user, link && text element.
                                    if (
                                        richTextElement.type === 'user' &&
                                        richTextElement.user_id ===
                                            slackEvent.context.botUserId
                                    ) {
                                        rawMessage = `${rawMessage}@${this.botName}`;
                                    } else if (
                                        richTextElement.type === 'text'
                                    ) {
                                        rawMessage =
                                            rawMessage + richTextElement.text;
                                    } else if (
                                        richTextElement.type === 'link'
                                    ) {
                                        rawMessage =
                                            rawMessage + richTextElement.url;
                                    }
                                }
    ...
    async processAction(
        slackEvent: SlackActionMiddlewareArgs & AllMiddlewareArgs
    ): Promise<void> {
        // Print start log
        ...
        try {
            ...
            // (<Record<string, any>>slackEvent.message).user is the id of the user
            let user = this.getUser(
                (<Record<string, any>>slackEvent.body).user.id
            ); // eslint-disable-line @typescript-eslint/no-explicit-any
            // if user have not been cached, then search from the slack server and cache it
            if (user === undefined) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const userInfo = await slackEvent.client.users.info({
                    user: (<Record<string, any>>slackEvent.body).user.id,
                });
                logger.debug(
                    `Cache the user info: ${JSON.stringify(userInfo)}`
                );
            ...
            if (segments.length >= 3) {
                ...
                logger.error(
                    `The data format of action_id is wrong!\n action_id=${actionId}`
                );
            }
            if (eventBody.type === 'view_submission') {
                ...
                if (eventBody.actions[0].type === 'static_select') {
                    ...
                    logger.error(
                        `Unsupported Slack interactive component: ${eventBody.actions[0].type}`
                    );
                }
            }
    ...
    async processViewAction(
        slackEvent: SlackViewMiddlewareArgs & AllMiddlewareArgs
    ): Promise<void> {
        // Print start log
        ...
        try {
            ...
            const privateMetaData = JSON.parse(
                slackEvent.payload.private_metadata
            );
    ...
    async getChannelById(
        ...
        try {
            const conversationInfo = await slackWebClient.conversations.info({
                channel: id,
            });

            let chattingType: IChattingType = IChattingType.UNKNOWN;
            if (
                conversationInfo.channel.is_channel === true &&
                conversationInfo.channel.is_mpim === false
            ) {
