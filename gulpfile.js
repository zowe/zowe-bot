...
if (
  ...
  console.log('');
  console.log(
    'The value of some required building system environment variables is empty!'
  );
...
function lintTask() {
  return gulp
    ...
    .pipe(gulp.src('test/**'))
    .pipe(
      gulpIf(isTypeScript, gulpEslint(), gulpIf(isJavaScript, gulpEslint()))
    )
...
function buildSourceTask() {
  return gulp
    .src(folder.src.source, {dot: true})
...
async function createPackageJsonTask() {
  ...
  if (nodeEnv === 'production') {
    ...
    return fs.writeFileSync(
      `./${folder.src.destination}package.json`,
...
async function packagingTask() {
  ...
  packagedFileName = `zowe-bot-v${releaseVersion.replace(
    /\./g,
    ''
  )}${releaseTypeSegment}${buildTypeSegment}-${buildTime}.tar.gz`;
  releasedFileName = `zowe-bot-v${releaseVersion.replace(/\./g, '')}.tar.gz`;

  if (nodeEnv === 'production') {
    // Product: folder.src.destination = 'dist/'
    return childProcess.execSync(
      `cd ./${folder.src.destination} && mkdir -p ../release && rm -rf ../release/zowe-bot*.tar.gz ` +
